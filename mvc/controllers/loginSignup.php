    ...
    class LoginSignup extends Controller{
        ...
        function index(){
            if (isset($_POST["login"])){
                ...
                if ($model->checkUser($email, $password)) {
                    return header("Location:http://localhost:8080/T%E1%BB%B1%20h%E1%BB%8Dc%20PHP/otakuvn/home");
                } else {
        ...
        function signup(){
            ...
            if($model->signUp($name, $phoneNumber, $address, $email, $password)){
                return header("Location: http://localhost:8080/T%E1%BB%B1%20h%E1%BB%8Dc%20PHP/otakuvn/loginSignup/login");
            }else{
